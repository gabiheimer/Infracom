...
public class Cliente1 {
    ...
    public static void main(String[] args) throws IOException, InterruptedException{
        ...
        gui interfaces = new gui(u1, InetAddress.getByName("localhost"), portaCliente2);
...
class Node {
    ...
    void enviar(InetAddress segundoNodeIP, int portaSegundoNode, gui interfaces) throws IOException {
        byte[] out = interfaces.output.getText().getBytes();
        DatagramPacket sendPacket = new DatagramPacket(out, out.length, segundoNodeIP, portaSegundoNode);
        nodeSocket.send(sendPacket);
        interfaces.output.setText(null);
        //System.out.println("Enviado!");
    }

    void receber(int numNode, gui interfaces) throws IOException {
        while (true) {
...
class Enviar extends Thread{
    ...
    public Enviar(Node node, InetAddress segundoNodeIP, int portaSegundoNode, gui interfaces) throws IOException {
        this.node = node;
...
class Receber extends Thread{
    ...
    public Receber(Node node, int numNode, gui interfaces) {
        this.node = node;
